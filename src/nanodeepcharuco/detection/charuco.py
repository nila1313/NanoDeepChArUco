...
from .nano import MarkerDetection
...
def load_board_parameters(
    ...
    if missing:
        raise ValueError(
            "Board configuration missing keys: "
            f"{sorted(missing)}"
        )
...
def build_charuco_board(
    ...
    board = cv2.aruco.CharucoBoard(
        ...
        float(
            params["square_size_real"]
        ),
        float(
            params["marker_size_real"]
        ),
        dictionary,
    )
...
def interpolate_charuco_from_nano(
    image: np.ndarray,
    marker_detections: list[
        MarkerDetection
    ],
    board,
) -> list[CharucoDetection]:
    """
    Convert ArUco Nano marker detections
    into standard ChArUco corners.

    ArUco Nano performs marker detection.

    OpenCV is used only for the ChArUco
    board geometry and interpolation.
    """
